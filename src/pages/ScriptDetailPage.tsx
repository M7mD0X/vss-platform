...
export default function ScriptDetailPage() {
  ...
  const loadstringUrl = script
    ? `${window.location.origin}${window.location.pathname}#api/script/${script.id}`
    : '';

  async function handleCopyLoadstring() {
    if (!loadstringUrl) return;
    try {
      await navigator.clipboard.writeText(`loadstring(game:HttpGet('${loadstringUrl}'))()`);
      setCopied(true);
  ...
  async function handleAddVersion(e: React.FormEvent) {
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : 'Failed to add version');
    } finally {
      setUploading(false);
    }
  }
  ...
  if (!script) {
    return (
      <div className="card p-8 text-center">
        ...
        <Link to="/scripts" className="mt-3 inline-block text-sm font-semibold text-accent-light">← Back to Scripts</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="mt-3 flex items-start justify-between">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-500">
              <span className="chip">{script.visibility}</span>
              {script.category && <span className="chip">{script.category}</span>}
              {script.game && <span className="chip">{script.game}</span>}
      ...
      {/* Description */}
      {script.description && <p className="text-sm text-slate-300">{script.description}</p>}

      {/* Tags */}
      {script.tags && script.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {script.tags.map(t => <span key={t} className="chip">#{t}</span>)}
        </div>
      )}
      ...
      <div className="card p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleCopyLoadstring} className={`btn-primary shrink-0 ${copied ? 'bg-success' : ''}`}>
            {copied ? '✓ Copied' : 'Copy'}
          </button>
        </div>
      ...
      <div className="card p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={() => setShowAddVersion(!showAddVersion)} className="text-xs font-semibold text-accent-light hover:underline">
            {showAddVersion ? 'Cancel' : '+ New Version'}
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {versions.map(v => (
            <button key={v.version_num} onClick={() => setSelectedVersion(v.version_num)}
              ...
              v{v.version_num}
              <span className="ml-1.5 text-[9px] text-slate-600">{new Date(v.created_at).toLocaleDateString()}</span>
            </button>
          ))}
        </div>
        ...
        {showAddVersion && (
          <form onSubmit={handleAddVersion} className="mt-4 space-y-3 border-t border-white/5 pt-4">
            <div>
              ...
              <input type="text" value={newChangelog} onChange={(e) => setNewChangelog(e.target.value)} className="input" placeholder="What changed in this version?" />
            </div>
            <div>
              <label className="label">New Source Code</label>
              <textarea value={newCode} onChange={(e) => setNewCode(e.target.value)} className="input font-mono text-xs" rows={10} placeholder="-- Paste updated Lua code" required />
            </div>
            <button type="submit" disabled={uploading || !newCode} className="btn-primary w-full">
              {uploading ? 'Uploading…' : 'Publish New Version'}
            </button>
          </form>
        )}
      </div>
      ...
      {currentVersion && (
        <div className="card overflow-hidden">
          <div className="flex items-center justify-between border-b border-white/5 px-4 py-2.5">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Source Code — v{currentVersion.version_num}</h2>
          ...
          <pre className="overflow-x-auto p-4 font-mono text-xs leading-relaxed text-slate-300" style={{ maxHeight: '500px' }}>
            <code>{currentVersion.source_code}</code>
          </pre>
        </div>
      )}
    </div>
